    ...
    <div id="page-wrapper">
        ...
        <div class="row">
            <div class="col-lg-4">
                ...
                <div class="form-group">
                    <label>Vendor</label>
                    <select class="form-control" type="number" id="vendor"></select>
                </div>
                <div><br>
                    <button onclick="create();" type="button" class="btn btn-outline btn-primary"
                            id="create-material-btn">Create
                    </button>
                </div>
            </div>
            <!-- /.col-lg-12 -->
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        loadVendors(function (vendors) {
            ...
            for (var i = 0; i < size; i++) {
                options += '<option value="' + vendors[i].id + '">' + vendors[i].firstName + ' ' + vendors[i].lastName + '</option>';
            }
            $('#vendor').html(options);
        })
    });
    ...
    var createRawMaterial = function (callback) {
        var rawMaterialData = {
            "name": $("#name").val(),
            "weight": $("#weight").val() || 0,
            "vendor": parseInt($("#vendor option:selected").val(), 10)
        };
        ...
        $.ajax({
            ...
            complete: function (createdRawMaterial) {
                if (callback) {
                    // createdRawMaterial
                    callback(createdRawMaterial);
                }
            }
        });
    }
